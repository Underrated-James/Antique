      ...
      upload_business_permit(req, res, (err) => {
        if (err) {
          return res.status(400).json({
            status: "error",
            message: "Error uploading business permit (PDF/image required).",
          });
        }
        next();
      });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (exists) {
        ...
        return res.status(409).json({ status: "error", message: "Email or phone already exists" });
      }
      ...
      if (role === "Seller") {
        if (!req.file) {
          await connection.rollback();
          return res.status(400).json({ status: "error", message: "Seller must upload business permit" });
        }
        finalBusinessPermit = `business_permits/${req.file.filename}`;
      }
      ...
      if (role === "Seller") {
        await connection.query(
          ...
          [
            ...
            phone.trim(),
            paypal || null,
            finalBusinessPermit, // <-- used as business_number
            finalBusinessPermit, // <-- used as business_permit_file
          ]
        );
      } else {
        await connection.query(
          ...
          VALUES (?, ?, ?, ?, ?, ?)`,
          [user_id, first_name.trim(), last_name.trim(), phone.trim(), address.trim(), email.trim()]
        );
      }
      ...
      console.error("❌ Signup SQL error:", err); // 👈 will show the real reason
      res.status(500).json({ status: "error", message: "Server error during signup", error: err.message });
    } finally {
      if (connection) connection.release();
    }
  }
...
app.post("/api/reset-password", async (req, res) => {
  ...
  try {
    ...
    // Check if user exists in accounts + customer_tb/seller_tb
    const [[userExists]] = await connection.query(
      `
      SELECT a.user_id, a.username, a.role, c.email as customerEmail, s.email as sellerEmail
      FROM accounts a
    ...
    // Match email against whichever table the user belongs to
    const matchedEmail =
      userExists.customerEmail || userExists.sellerEmail || null;

    if (!matchedEmail || matchedEmail.trim().toLowerCase() !== email.trim().toLowerCase()) {
      await connection.rollback();
      return res.status(400).json({
        status: "error",
    ...
    await connection.query(
      ...
      [hashedPassword, userExists.user_id]
    );
...
app.get("/api/profile/:userId", async (req, res) => {
  ...
  try {
    ...
    const [accountRows] = await connection.query(
      "SELECT username, email, role FROM accounts WHERE user_id = ?",
      [userId]
    );
    ...
    let profileData = { ...account };

    if (account.role === "customer") {
      const [customerRows] = await connection.query(
        "SELECT profile_pic, First_name, Last_name, phone_num, Address FROM customer_tb WHERE customer_id = ?",
        [userId]
      );

      if (customerRows.length > 0) {
        let profilePicPath = customerRows[0].profile_pic;
        ...
        profileData = {
          ...profileData,
          ...customerRows[0],
          profile_pic: profilePicPath || null,
        };
      }
    } else if (account.role === "seller") {
      const [sellerRows] = await connection.query(
        "SELECT seller_image, Business_Permit, First_name, Last_name, phone_num, Address FROM seller_tb WHERE seller_id = ?",
        [userId]
      );

      if (sellerRows.length > 0) {
        let sellerImagePath = sellerRows[0].seller_image;

        if (sellerImagePath && !sellerImagePath.startsWith("uploads/")) {
          sellerImagePath = `uploads/${sellerImagePath}`;
        }

        if (sellerImagePath) {
          sellerImagePath = sellerImagePath.replace(/\\/g, "/");
        }

        profileData = {
          ...profileData,
          ...sellerRows[0],
          profile_pic: sellerImagePath || null, // map seller_image to profile_pic
        };
      }
    } else {
...
app.get("/api/item-detail/:id", async (req, res) => {
  ...
  try {
    ...
    // Helper to strip "uploads/" if already included and return proper URL
    const formatImage = (path) => {
      if (!path) return "/uploads/Vinque_logo.png"; // fallback
      const normalized = path.replace(/^uploads[\\/]+/, ""); // remove leading uploads/ or uploads\
    ...
    res.json({
      ...
      data: {
        ...
        Historian_Name: product.Historian_Name,
        Historian_Type: product.Historian_Type,
        ...
        store_image: formatImage(seller.store_image),
      },
    });
  } catch (err) {
...
app.get("/api/header/search", async (req, res) => {
  ...
  try {
    ...
    const sellerId = req.query.sellerId || req.query.seller_id || null;
    ...
    const updatedRows = rows.map((item) => ({
      ...item,
      image1_path: item.image1_path?.replace(/^uploads[\\/]+/, "") || null,
    }));

    // ✅ Get profile image depending on role
    let profilePic = null;
    ...
    if (customerId) {
      ...
      if (profileRows[0]?.profile_pic) {
        profilePic = profileRows[0].profile_pic.replace(/^uploads[\\/]+/, "");
      }
    }

    if (sellerId) {
      const [profileRows] = await connection.query(
        "SELECT seller_image FROM seller_tb WHERE seller_id = ?",
        [sellerId]
      );
      if (profileRows[0]?.seller_image) {
        sellerImage = profileRows[0].seller_image.replace(/^uploads[\\/]+/, "");
      }
    }
    ...
    res.json({
      status: "success",
      data: updatedRows,
      profile_pic: profilePic,   // ✅ always customer only
      seller_image: sellerImage, // ✅ always seller only
    });
  } catch (err) {
...
app.put("/api/seller/update/:id", (req, res) => {
  upload_profile(req, res, async (err) => {
    ...
    const hasNewImage = req.files?.profile_image?.length > 0;
    ...
    try {
      ...
      if (!sellerResult.length) {
        if (seller_image) fs.unlink(req.files.profile_image[0].path, () => {});
        return res.status(404).json({ success: false, error: "Seller not found" });
      }

      // Prepare update
      let updateQuery = "UPDATE seller_tb SET business_name = ?, business_description = ?, business_address = ?, phone_num = ?";
      ...
      // Determine image URL to return
      let imageUrl = seller_image ? `/uploads/${seller_image}` : profile_pic_url || sellerResult[0].seller_image;
      if (imageUrl && !imageUrl.startsWith("http") && !imageUrl.startsWith("/uploads/")) imageUrl = `/uploads/${imageUrl}`;
